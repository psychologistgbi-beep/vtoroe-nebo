(() => {
  ...
  const SCENES = {
    phyllo: { kind: 'growth', label: 'прорастание световых семян', event: 'Иногда семена начинают подниматься из рисунка. Спираль при этом не ломается: подъём идёт тем же золотым углом, что и рост.', colors: ['#f7dd83', '#e86e45'], count: 34 },
    ...
    vitel: { kind: 'tracers', label: 'течение невидимой воды', event: 'Ненадолго появляются светлые трассеры. Огибают скрытые вихри и гаснут; карта течения остаётся неполной.', colors: ['#8ee8e5', '#d7fff3'], count: 28 },
    frost: { kind: 'growth', label: 'приход холодного сада', event: 'От горизонта прорастают новые ветви. Каждое прилипание меняет дороги всех следующих частиц.', colors: ['#dff7ff', '#8bb8dd'], count: 42 },
    mayatnik: { kind: 'tracers', label: 'расхождение судеб', event: 'Два следа стартуют почти вместе и расходятся навсегда. В расчёте нет ни ошибки, ни случайности.', colors: ['#ffcb77', '#e96d8a'], count: 16, chaos: true },
    ...
    beam: { kind: 'wave', label: 'дыхание дифракции', event: 'Из зенита расходится кольцо. За ясной полосой глухая зона, потом ещё один слабый ответ: прибор проверяет собственный слух.', colors: ['#e6f7ff', '#80b8d8'], count: 9 },
    ...
    apollon: { kind: 'orbs', label: 'рождение вложенных миров', event: 'В промежутках появляются меньшие круги, затем ещё меньшие. Заполнить всё не выйдет: каждый зазор открывает следующий.', colors: ['#f2c96d', '#ef8e72'], count: 18 },
    caustic: { kind: 'storm', label: 'звёздный световой шквал', event: 'Складки света сходятся в яркий гребень и проходят по своду. Невидимая масса на секунду показывает свой почерк.', colors: ['#fff5bf', '#ff936b'], period: 43, fierce: true },
    ...
    superfluid: { kind: 'storm', label: 'смертельная магнитная буря', event: 'Вихри стягивают небо в поле заряженных дуг. Буря проходит близко и бесшумно.', colors: ['#91eaff', '#e35cff'], period: 47, fierce: true },
    ...
    equal_area: { kind: 'breath', label: 'равное дыхание ячеек', event: 'Ячейки дышат: на картинке по-разному, по площади сферы одинаково. Купол время от времени проверяет свою арифметику.', colors: ['#cfddb0', '#e9b875'], mode: 8 },
    ...
    parallax_covenant: { kind: 'tracers', label: 'маяки несводимых взглядов', event: 'Три семейства маяков смещаются независимо. Цельная картина собирается только из одной точки зала и распадается, стоит сдвинуться.', colors: ['#83d7ef', '#f1af7c'], count: 21 },
    ...
    portal_1: {
      ...
      event: 'Все 84 фигуры на своих старых местах, я не добавил ни одной. Внешний ярус первым теряет вес, движение передаётся внутрь, и сонм ненадолго тянется к зениту.',
      cues: [
        ...
        { at: 66, label: 'остаток · сонм снова на местах, но легче' }
      ]
    },
    portal_2: {
